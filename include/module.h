...
namespace Module {
    ...
    class FastDrawTH2D : public Module {
    private:
        TH2D* hist;
        std::string hist_title;
        int x_nbins;
        ...
        std::vector<std::string>* variable_names;
        std::vector<std::string>* VariableTypes;
        std::string x_variable_name;
        int x_variable_index;
        std::string y_variable_name;
        int y_variable_index;
    ...
    public:
        ...
        int Process(std::vector<Data>* data) override {
            for (std::vector<Data>::iterator iter = data->begin(); iter != data->end(); ) {
                ...
                if (std::holds_alternative<double>(iter->variable.at(x_variable_index))) x_temp_ = (double)std::get<double>(iter->variable.at(x_variable_index));
                else if (std::holds_alternative<int>(iter->variable.at(x_variable_index))) x_temp_ = (double)std::get<int>(iter->variable.at(x_variable_index));
                else if (std::holds_alternative<unsigned int>(iter->variable.at(x_variable_index))) x_temp_ = (double)std::get<unsigned int>(iter->variable.at(x_variable_index));
                else if (std::holds_alternative<float>(iter->variable.at(x_variable_index))) x_temp_ = (double)std::get<float>(iter->variable.at(x_variable_index));
                else {
                    printf("unknown type of variable\n");
                    exit(1);
                }
                ...
                if (std::holds_alternative<double>(iter->variable.at(y_variable_index))) y_temp_ = (double)std::get<double>(iter->variable.at(y_variable_index));
                else if (std::holds_alternative<int>(iter->variable.at(y_variable_index))) y_temp_ = (double)std::get<int>(iter->variable.at(y_variable_index));
                else if (std::holds_alternative<unsigned int>(iter->variable.at(y_variable_index))) y_temp_ = (double)std::get<unsigned int>(iter->variable.at(y_variable_index));
                else if (std::holds_alternative<float>(iter->variable.at(y_variable_index))) y_temp_ = (double)std::get<float>(iter->variable.at(y_variable_index));
                else {
                    printf("unknown type of variable\n");
                    exit(1);
                }
                ...
                x_variable.push_back(x_temp_);
                y_variable.push_back(y_temp_);
                weight.push_back(ObtainWeight(iter));
    ...
    class FastDrawFOM : public Module {
    private:
        std::string variable_name;
        int variable_index;
        ...
        double MyEPSILON;
    public:
        FastDrawFOM(const char* variable_name_, double MIN_, double MAX_, const char* png_name_, std::vector<std::string> Signal_label_list_, std::vector<std::string> Background_label_list_, std::vector<std::string>* variable_names_, std::vector<std::string>* VariableTypes_) : Module(), variable_name(variable_name_), MIN(MIN_), MAX(MAX_), png_name(png_name_), Signal_label_list(Signal_label_list_), Background_label_list(Background_label_list_), variable_names(variable_names_), VariableTypes(VariableTypes_) {
            // just 50
            NBin = 50;
        ...
        int Process(std::vector<Data>* data) {
            ...
            for (int i = 0; i < NBin; i++) {
                ...
                for (std::vector<Data>::iterator iter = data->begin(); iter != data->end(); ) {
                    ...
                    if (temp_ > variable_value) DoesItPassCriteria = true;
                    else DoesItPassCriteria = false;
    ...
    class FastDrawStack : public Module {
    private:
        THStack* stack;
        TH1D* stack_error;
        TH1D* hist;
    ...
    public:
        FastDrawStack(const char* variable_name_, const char* stack_title_, int nbins_, double x_low_, double x_high_, const char* png_name_, std::vector<std::string> Signal_label_list_, std::vector<std::string> Background_label_list_, std::vector<std::string> data_label_list_, std::vector<std::string> MC_label_list_, std::vector<std::string>* variable_names_, std::vector<std::string>* VariableTypes_) : Module(), variable_name(variable_name_), stack_title(stack_title_), nbins(nbins_), x_low(x_low_), x_high(x_high_), png_name(png_name_), Signal_label_list(Signal_label_list_), Background_label_list(Background_label_list_), data_label_list(data_label_list_), MC_label_list(MC_label_list_), variable_names(variable_names_), VariableTypes(VariableTypes_) {}
        FastDrawStack(const char* variable_name_, const char* stack_title_, const char* png_name_, std::vector<std::string> Signal_label_list_, std::vector<std::string> Background_label_list_, std::vector<std::string> data_label_list_, std::vector<std::string> MC_label_list_, std::vector<std::string>* variable_names_, std::vector<std::string>* VariableTypes_) : Module(), variable_name(variable_name_), stack_title(stack_title_), nbins(50), x_low(std::numeric_limits<double>::max()), x_high(std::numeric_limits<double>::max()), png_name(png_name_), Signal_label_list(Signal_label_list_), Background_label_list(Background_label_list_), data_label_list(data_label_list_), MC_label_list(MC_label_list_), variable_names(variable_names_), VariableTypes(VariableTypes_) {}
        ...
        void Start() override {
            ...
            if ((data_label_list.size() != 0) && (MC_label_list.size() != 0)) {}
                ...
                printf("`FastDrawStack` requires one of them:\n");
                printf("1. `SetMC` and `SetData`\n");
                printf("2. `SetSignal` and `SetBackground`\n");
                printf("3. `SetMC` only\n");
                exit(1);
